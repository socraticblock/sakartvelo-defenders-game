...
const targetFile = path.join(__dirname, '../src/EnemyBuilders.ts');
...
content = content.replace(/new THREE\.SphereGeometry\(([^)]+)\)/g, 'geoCache.getSphere($1)');
content = content.replace(/new THREE\.IcosahedronGeometry\(([^)]+)\)/g, 'geoCache.getIcosahedron($1)');

fs.writeFileSync(targetFile, content);
console.log('Successfully replaced geometries with cache in EnemyBuilders.ts');
